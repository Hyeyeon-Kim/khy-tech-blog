...
처음에는 cake마다 store를 하나씩 조회했다. 구현은 단순했지만, cake 수가 늘어날수록 store 조회도 같이 늘어났다. 같은 store를 여러 cake가 참조해도 매번 다시 조회했다.

## 1. 문제 상황: cake 수만큼 store 조회가 늘어났다

유사 케이크 조회 API의 응답에는 cake 정보뿐 아니라 해당 cake를 판매하는 store 정보도 필요했다.

응답에 필요한 store 정보는 store 이름, 주소, taste, 위도, 경도였다. 하지만 외부 AI 검색 서버가 내려주는 cake 데이터에는 store 전체 정보가 아니라 `owner_store_id`만 들어있었다.
...
또한 여러 cake가 동일한 store를 참조하는 경우에는 매번 개별 조회가 수행되어, 불필요한 중복 쿼리가 발생한다.

## 2. 먼저 결론: `$in` batch를 활용해 동일 store 중복 조회를 제거

최종적으로는 `$in` batch를 선택했다.
...
핵심은 반복문 안에서 DB 조회를 하지 않는 것이다. 반복문은 응답을 조립하는 역할만 하고, store 조회는 반복문 밖에서 한 번에 처리한다.

이렇게 바꾸면 store 조회는 cake 개수만큼 늘어나지 않는다. 같은 store가 여러 번 등장해도 `Set`이 중복을 제거하므로 조회 대상에는 한 번만 들어간다.

측정 결과 size=100, 중복도 low 기준 current의 hydration P95는 9.25ms였고, `$in` batch는 3.12ms였다. 중복도 high에서는 batch가 0.66ms까지 내려갔다.

가장 빠른 방식은 아니었지만 현재 조건에서는 `$in` batch가 충분히 빨랐고, 코드 변경 범위와 운영 부담이 가장 작아서 선택했다.

## 3. 검토한 해결책 3가지

검토했던 다른 해결책 후보는 3가지였다.
...
각 방식은 모두 같은 문제를 해결할 수 있지만 해결하는 위치가 다르다.

1. `$in` batch는 애플리케이션에서 필요한 store id를 모은 뒤 DB에 한 번만 요청한다. 현재 코드 흐름을 크게 바꾸지 않고 N+1을 제거할 수 있다.

2. `$lookup`은 MongoDB aggregate에서 cake와 store를 조인한다. DB 안에서 한 번에 처리할 수 있지만, 현재 유사 케이크 조회 API에서는 외부 AI 검색 서버가 이미 cake 객체를 완성해서 NestJS에 넘겨준다. 이 상태에서 `$lookup`을 쓰려면 받은 cake를 다시 MongoDB에서 조회하거나, AI 서버의 응답 계약을 바꿔야 한다.
...
반복 조건은 iter 100회, warmup 10회였다. size는 10, 50, 100을 중심으로 봤고, batch 한계를 보기 위해 stress test에서는 200, 500, 1000까지 올렸다.
...
첫 번째는 Mongoose 레이어다. `mongoose.set('debug', fn)`로 Model 호출을 캡처했다. 애플리케이션 코드에서 store 조회가 몇 번 발생하는지 확인하기 위한 기준이다.
...
이 두 층을 같이 본 이유는 ORM의 추상화만 믿으면 실제 비용을 놓칠 수 있기 때문이다. Mongoose에서는 쿼리 1번으로 보이더라도, MongoDB wire-level에서는 추가 command가 발생할 수 있다.
...
uniform에서 size=1000일 때 batch P95가 31ms까지 올라갔다. 하지만 같은 size에서도 zipf는 13.16ms, hotspot은 5.72ms였다.

결국 batch 비용을 결정하는 것은 cake 수 자체보다 unique store 수였다. 현실 트래픽은 완전한 uniform보다는 특정 인기 매장에 몰리는 zipf나 hotspot에 가까울 가능성이 높다고 봤다.
...
이 부분은 `.lean()`을 적용하면 줄일 수 있는 여지가 있다. 다만 이번 측정은 운영 코드와 같은 조건을 보기 위해 그대로 두었다.
...
store 정보가 바뀌면 그 store를 참조하는 모든 cake 문서를 함께 갱신해야 한다. 즉, 읽기 경로의 비용을 쓰기 경로로 옮기는 선택이다.

예를 들어 store 이름이나 주소가 바뀌었는데 cake 안의 snapshot을 갱신하지 못하면, 응답에는 오래된 store 정보가 내려갈 수 있다. store update와 cake `updateMany` 사이에 짧은 불일치 구간이 생길 수 있고, 트랜잭션을 쓰지 않는다면 이 일관성 책임은 애플리케이션이 떠안아야 한다.
...
그래서 비정규화는 나빠서 제외한 것이 아니다. batch가 충분히 빨랐기 때문에, 쓰기 fan-out과 애플리케이션 레벨 일관성 책임을 새로 도입할 이유가 작았다.

## 7. 왜 `$lookup`을 선택하지 않았나

`$lookup`도 N+1을 줄일 수 있다. wire-level command 수만 보면 batch와 마찬가지로 1회로 묶인다.

하지만 이번 API에서는 `$lookup`이 구조적으로 자연스럽지 않았다.
...
이 상태에서 `$lookup`을 쓰려면 두 가지 중 하나를 해야 한다.
...
즉, `$lookup`을 선택하지 않은 핵심 이유는 단순히 느려서가 아니었다. 현재 시스템 경계와 맞지 않았기 때문이다.
...
결론적으로 `$lookup`은 성능 수치와 별개로 현재 API 흐름과 책임 경계에 맞지 않았다.

## 8. 최종적으로 `$in` batch를 선택한 이유

최종 선택은 `$in` batch였다.

이 방식은 가장 빠른 선택지는 아니었다. 읽기 성능만 보면 비정규화가 더 빠르다. DB 안에서 조인까지 처리한다는 관점에서는 `$lookup`도 후보가 될 수 있다.

하지만 이번 조건에서 중요한 기준은 "가장 빠른 방식"이 아니라 "현재 구조에서 충분히 빠르면서 운영 부담이 작은 방식"이었다.

`$in` batch를 선택한 이유는 다음과 같다.

- DB 왕복을 N회에서 1회로 줄일 수 있다.
- 같은 store의 중복 조회를 제거할 수 있다.
...
batch는 모든 비교 차원에서 압도적인 1등은 아니었다. 대신 반대 방향의 큰 손해가 없었다.
...
이번 선택은 현재 운영 범위와 현재 API 구조 안에서의 결론이다. 아래 조건이 바뀌면 다시 검토해야 한다.
...
결정 기준은 단순히 "P95가 몇 ms 이하인가"가 아니었다. 어떤 조건이 바뀌면 어떤 선택지를 다시 볼 것인지까지 정해두는 것이 더 중요했다.
